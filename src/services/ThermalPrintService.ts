...
class ThermalPrintService {
  ...
  private constructor() {
    // Singleton pattern - load the active printer from settings
    this.loadActivePrinter();
  }

  /**
   * Load active printer from settings and localStorage fallback
   */
  private async loadActivePrinter() {
    try {
      // Try to load from settings via API first
  ...
  /**
   * Get all available printers in the system with optional cache refresh
   */
  public async getAvailablePrinters(forceRefresh = false): Promise<Printer[]> {
    try {
      ...
      // Updated thermal-printer detection
   ...
   * @param printerName The name of the printer to set as active
   */
  ...
  /**
   * New helper for thermal detection
   */
  ...
  /**
   * Check if printer module is loaded
   */
  public static isPrinterModuleAvailable(): boolean {
    return !!(window.printerApi?.getPrinters || window.api?.getPrinters);
  }

  /**
   * Send raw ESC/POS commands to the printer with enhanced error handling
   * and dynamic API detection
   */
  public async printRaw(
    ...
    try {
      ...
      if (window.printerApi?.printRaw) {
        ...
        // Convert Uint8Array back to string for window.api
        const decoder = new TextDecoder();
        const stringData = decoder.decode(printData);
        return await window.api.printRaw(stringData, targetPrinter);
      } else {
  ...
  /**
   * Test printer by sending a simple test page with ESC/POS commands
   */
  ...
  /**
   * Print a receipt with better ESC/POS command generation
   */
  public async printReceipt(
    ...
    try {
      // Use provided printer or active printer
      const targetPrinter = printerName || this.activePrinter;
      ...
      // Create ESC/POS commands array for better control
      const commands = [];
  ...
  /**
   * Check printer status with enhanced detection
   */
